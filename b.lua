...
ActivationFuncs.MO = function(x, d)
...
function module:CreateNN(NumberInputs, NumberHidden, NumberHLayers, NumberOutputs, LearningRate, ActivationFunction)
    ...
    for layer = 1,NumberHLayers,1 do
        ...
        for node = 1,NumberHidden,1 do
            NewNN.__Bias[layer][node] = random(-1,1)
        end
    end
...
targetblock.Position = Vector3.new(0, 5, 0)
targetblock.Size = Vector3.new(random(0, maxb), random(0, maxb), random(0, maxb))
targetblock.Transparency = math.random()
targetblock.Color = Color3.fromRGB(random(0, 255), random(0, 255), random(0, 255))
...
for i = 1,ts do
	...
	local transparency = things[4]
	local color = Color3.new(math.clamp(things[5], 0, 1), math.clamp(things[6], 0, 1), math.clamp(things[7], 0, 1))
	nnblock.Size = size
	nnblock.Transparency = transparency
	nnblock.Color = color
	wait()
end
